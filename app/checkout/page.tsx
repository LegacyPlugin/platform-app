    ...
    const handleUpdateEmail = async () => {
        ...
        try {
            const token = localStorage.getItem("legacy_token")
            const res = await fetch("/api/v1/client/email", { 
                method: "POST",
                headers: { 
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}` 
            ...
            if (res.ok) {
                alert("Email vinculado com sucesso!")
                setIsEditingEmail(false)
                
                window.location.reload()
            } else {
                const data = await res.json().catch(() => ({}))
                alert(data.message || "Erro ao atualizar email.")
            }
        } catch (e) {
            console.error(e)
            alert("Erro ao conectar com o servidor.")
                            ...
                            {!isEditingEmail && profile?.email ? (
                                <div className="flex items-center gap-2 group">
                                    ...
                                    <button 
                                        ...
                                        className="opacity-0 group-hover:opacity-100 transition text-zinc-600 hover:text-purple-400"
                                    >
                                        <Edit2 size={12} />
                                    </button>
                                </div>
                                    ...
                                    {!isEditingEmail ? (
                                        ...
                                        <div className="flex items-center gap-1">
                                            <input 
                                                ...
                                                className="w-40 bg-black/50 border border-white/10 rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-purple-500"
                                            />
                                            <button 
                                                onClick={handleUpdateEmail}
                                                disabled={isSavingEmail}
                                                className="p-1 hover:bg-green-500/10 text-zinc-400 hover:text-green-400 rounded transition"
                                            >
